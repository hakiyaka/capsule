...
| `insight` | optional time window; `history:true`, `deep:true`, `compaction:true`, and bounded limits | Local savings and environment analysis; optional metadata-only or complete hash-only line audit plus adaptive context-pressure report |
...
SQLite uses WAL plus a bounded busy timeout so short-lived writers from hooks, searches, and indexing can coexist across processes. Environment diagnostics are advisory: Capsule never moves or restores skills without an explicit `skills apply|restore` request and `confirm:true`. `insight {history:true}` reads only the first session metadata record and file size from bounded Codex session catalogs; it does not parse prompts or tool outputs. For an explicit exhaustive local audit, add `deep:true` (or `line_scan:true`): Capsule streams every line of discovered active and archived JSONL, counts exposed token/tool/error/compaction signals, and returns only sizes, categories, counters, and one-way hashes. It never persists transcript text or alters session files. The standalone `npm run audit:sessions` command exposes the same scan with explicit file/byte bounds.
...
The plugin ships `hooks/hooks.json` for Codex-native discovery through the current `features.hooks` flag. That bundle is the default. `scripts/install-hooks.cjs install` automatically removes its global fallback when native hooks are enabled; otherwise it installs an idempotent global fallback that resolves the current user and `CODEX_HOME`, preserves unrelated hooks, and replaces prior token-efficiency hooks. Never keep both sources active because Codex runs matching hooks from every source. Both paths provide pre/post-tool capture, session resume, and pre-compact handoff. Automatic event, phase, and final-memory capture is disabled unless `CAPSULE_CAPTURE_MEMORY=1`; when enabled it is sanitized and bounded. Raw prompt persistence is separately disabled unless `CAPSULE_CAPTURE_PROMPTS=1`. Migrated-context auto-injection is disabled unless `CAPSULE_INCLUDE_MIGRATED=1`.
...
If ordinary compaction cannot safely shrink a payload, the pressure circuit emits a bounded secret-redacted evidence envelope with raw size, SHA-256, and exact capsule ID instead of allowing one tool result to overflow the thread. It applies at critical/emergency pressure, at high pressure above twice the active maximum, and at normal pressure for eligible read-only output above 32,000 characters. An independent universal hard cap also envelopes any non-failed tool result above 512,000 characters, including tools whose mutation semantics are unknown; the exact capsule remains recoverable. Exact evidence stays expandable, pagination remains available, and `includeOutputs:true` bypasses both thread preflight bounding and projection. A compaction replacement history of at least 128,000 serialized characters (or 256,000 across the bounded recent tail; 512,000/1,000,000 raises to critical) also raises the next-turn pressure policy, so repeated replacement churn is cut before another large result is admitted. Configure the normal cap with `CAPSULE_ABSOLUTE_OUTPUT_CHARS=16000..1000000`; configure or disable the universal guard with `CAPSULE_UNIVERSAL_HARD_CAP_CHARS=64000..4000000` and `CAPSULE_UNIVERSAL_HARD_CAP=0`; set `CAPSULE_ABSOLUTE_OUTPUT=0`, `CAPSULE_THREAD_PREFLIGHT=0`, or `CAPSULE_THREAD_PROJECTION=0` for independent opt-outs.
...
Call `insight` with `compaction:true` plus `session` or `session_file` to audit observable transitions and receive `context_pressure`. The compact default reports aggregates and only the latest event; pass `compaction_events:true` and optional `compaction_event_limit` when exact bounded rows are needed. Session identifiers and absolute session paths are omitted from telemetry responses unless `include_identity:true` (or `CAPSULE_INCLUDE_SESSION_METADATA=1`) is explicitly requested. Codex currently does not expose the compactor model's own generation usage in session telemetry, so a zero adjacent delta is reported as unexposed, never as proof that compaction was free. These figures are context exposure, not billing claims.
...
Self-contained subagent tasks are automatically changed from an omitted or `all` fork to `none`. English deictic continuations use a three-turn recent window; explicit requests for the whole conversation preserve full history with a measured warning. Explicit `none` or windows up to five turns remain untouched. Spawn-count checkpoints are included in no-progress guidance. Set `CAPSULE_FORK_POLICY=observe` for warnings only or `off` to disable the policy. The first Codex restart may request trust for the new local commands.

Task-wide delegation is bounded separately from the per-mutation fuse: the default total is 16 subagents and the hard configurable maximum is 20. UI, i18n, performance, and release lanes accept one agent by default; `capsule_lane_revisit:true` is reserved for a distinct verification need. After 10 independent spawns, the next lane requires one compact `capsule_phase_summary` containing `decisions`, `evidence`, `files`, and `blockers`; the summary is hashed locally and its raw text is not persisted. Subagent outputs are projected as compact digests and must omit screenshots, base64 media, full diffs, and transcripts; exact evidence remains recoverable by capsule id. After each grouped mutation plus decisive verification, call `stats` and `gain` once, then stop when the acceptance condition is proven.
