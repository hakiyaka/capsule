...
The plugin ships `hooks/hooks.json` for Codex-native discovery. That bundle is the default. `scripts/install-hooks.cjs install` is an idempotent global fallback only for standalone use: it resolves the current user and `CODEX_HOME`, preserves unrelated hooks, and replaces prior Capsule hooks. Never keep both sources active because Codex runs matching hooks from every source; use `hooks:remove-global` to preserve unrelated global hooks. Both paths provide pre/post-tool capture, session resume, and pre-compact handoff. Automatic event, phase, and final-memory capture is disabled unless `CAPSULE_CAPTURE_MEMORY=1`; when enabled it is sanitized and bounded. Raw prompt persistence is separately disabled unless `CAPSULE_CAPTURE_PROMPTS=1`.
...
If ordinary compaction cannot safely shrink a payload, the pressure circuit emits a bounded secret-redacted evidence envelope with raw size, SHA-256, and exact capsule ID instead of allowing one tool result to overflow the thread. It applies at critical/emergency pressure, at high pressure above twice the active maximum, and at normal pressure for eligible read-only output above 32,000 characters. An independent universal hard cap also envelopes any non-failed tool result above 512,000 characters, including tools whose mutation semantics are unknown; the exact capsule remains recoverable. A compaction replacement history of at least 128,000 serialized characters (or 256,000 across the bounded recent tail) raises the next-turn pressure policy before another large result is admitted. Exact evidence stays expandable, pagination remains available, and `includeOutputs:true` bypasses both thread preflight bounding and projection. Configure the normal cap with `CAPSULE_ABSOLUTE_OUTPUT_CHARS=16000..1000000`; configure or disable the universal guard with `CAPSULE_UNIVERSAL_HARD_CAP_CHARS=64000..4000000` and `CAPSULE_UNIVERSAL_HARD_CAP=0`; set `CAPSULE_ABSOLUTE_OUTPUT=0`, `CAPSULE_THREAD_PREFLIGHT=0`, or `CAPSULE_THREAD_PROJECTION=0` for independent opt-outs.
